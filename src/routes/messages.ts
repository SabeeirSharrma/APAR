  ...
  const enriched = messages.map((m) => {
    ...
    if (m.sender_type === 'interviewer') {
      const interviewer = getOne<{ name: string }>(
        'SELECT name FROM interviewers WHERE id = ?',
        { id: m.sender_id },
      );
      senderName = interviewer?.name || 'Interviewer';
    } else {
      // Applicant sender — use the application's name
      senderName = app ? getOne<{ name: string }>('SELECT name FROM applications WHERE id = ?', { id: applicantId })?.name || 'Applicant' : 'Applicant';
    }
    return { ...m, senderName };
  });
...
router.post('/', async (req: Request, res: Response) => {
  ...
  const assignment = getOne<{ id: string }>(
    'SELECT id FROM applications WHERE id = ? AND assigned_interviewer_id = ?',
